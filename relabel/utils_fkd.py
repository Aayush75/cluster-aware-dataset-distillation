...
class ImageFolder_FKD_MIX(torchvision.datasets.ImageFolder):
    def __init__(self, fkd_path, mode, weights_map, args_epoch=None, args_bs=None, **kwargs):
        ...
        self.mix_config = None  # mix config for current batch
        if self.mode == 'fkd_load' and self.fkd_path is not None:
    ...
    def __getitem__(self, index):
        ...
        weight = self.weights_map.get(path, 1.0)  # Default weight 1.0 if not in map
        
        if self.mode == 'fkd_save':
            coords_ = None
            flip_ = None
        elif self.mode == 'fkd_load':
            if self.batch_config is None or self.batch_config_idx == 0:
    ...
    def load_batch_config(self, img_idx):
        ...
        # [coords, flip_status, mix_index, mix_lam, mix_bbox, soft_label, weights]
        config = torch.load(batch_config_path, weights_only=False)
        self.batch_config_idx = 0
        self.batch_config = config[:2]
        self.last_mix_config = config[2:]  # Store for retrieval
        return config[2:]
